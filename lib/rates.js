...
/**
 * The vendors worth showing first, most familiar first.
 *
 * The live list runs to 59 vendors and most of them are one-model publishers
 * nobody is shopping for; a table of 59 is not a price list either. This order
 * is what picks the 15, and it is a deliberate editorial choice rather than a
 * computed ranking: sorting by model count alone puts `qwen` above `anthropic`
 * and would drift every time a publisher ships a batch of models. Every id here
 * was read off the live list rather than guessed — `meta-llama` and `x-ai` are
 * the real ids, not `meta` and `xai`.
 */
