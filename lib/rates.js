 ...
 *
 * Being stored and being marked are two different things. A stored value that
 * matches the fetched one keeps the fetched value's `fetched` source: the mark
 * says a refresh cannot change the row, which is only worth saying when the row
 * is not already what a refresh would produce.
 *
 * A typed value that does *not* match what the catalogue publishes is a
 * disagreement the user should get to settle, and settling it needs both numbers
 * — which only exists here, where the fetched value has not yet been overwritten
 * by the typed one. Those are collected into `pending`, one entry per model, so
 * the page can offer to adopt the official number or to keep the typed one.
 * Keeping it is recorded in the override itself, as `ack`: the official numbers
 * the user had already seen when they decided. A disagreement is then worth
 * raising again exactly when the official number moves away from that record.
 *
 ...
 * @returns {{ vendors: object[], fx: object, overriddenModels: number, fxOverridden: boolean, pending: object[] }} the effective view, and the disagreements left to settle.
 */
export function applyOverrides({ vendors = [], fx = undefined, overrides = {} } = {}) {
  ...
  // A model priced per period appears once per period under one id, and a single
  // typed value covers every one of those rows, so the review list is keyed by id:
  // a disagreement is a fact about the model, not about a row. The first row that
  // disagrees supplies the official number to show.
  ...
  const merged = vendors.map((entry) => ({
    ...
    models: entry.models.map((model) => {
      ...
      const keys = ['input', 'output', 'cacheRead', 'cacheWrite'].filter((key) => key in override)
      const ack = override.ack !== undefined && override.ack !== null && typeof override.ack === 'object' ? override.ack : {}
      const conflicts = []
      for (const key of keys) {
        const manual = toPrice(override[key])
        const official = toPrice(model.prices?.[key])
        // Nothing to hand the row back to when the catalogue publishes nothing
        // for this bucket, and a value already reviewed at this very number is
        // not news.
        if (official === null || manual === official || toPrice(ack[key]) === official) continue
        conflicts.push({ key, manual, official })
      }
      if (conflicts.length > 0 && !review.has(model.id)) {
        review.set(model.id, { id: model.id, name: model.name ?? model.id, vendor: entry.vendor ?? '', fields: conflicts })
      }
      // Re-typing the number the catalogue already shows is not a claim on the
      // row. The flag means "a refresh will not change this", and a row a
      // refresh would reproduce unchanged does not need protection, so only a
      // real difference earns it.
      if (!keys.some((key) => toPrice(override[key]) !== toPrice(model.prices?.[key]))) {
        return { ...model, source: 'fetched' }
      }
      overriddenModels += 1
      const prices = { ...model.prices }
      for (const key of keys) prices[key] = toPrice(override[key])
  ...
  return { vendors: merged, fx: effectiveFx, overriddenModels, fxOverridden: fxRate !== null && fxRate !== 0, pending: [...review.values()] }
}

/** The four price buckets, in the order they are read and written everywhere. */
export const PRICE_KEYS = ['input', 'output', 'cacheRead', 'cacheWrite']

/**
 * Validate an `adopt`- or `keep`-shaped map: model id to the price keys decided.
 *
 * These name keys rather than carry numbers, because the host is the one that
 * knows what the catalogue publishes; a page that sent its own copy of the
 * official price could write a stale number into the record of what was reviewed.
 *
 * @param {unknown} value - the raw map.
 * @param {string} field - the body field it came from, for the message.
 * @returns {{ ok: true, value: object }|{ ok: false, error: string }} the accepted map.
 */
function validateReviewKeys(value, field) {
  if (typeof value !== 'object' || value === null || Array.isArray(value)) return { ok: false, error: `${field} must be an object` }
  const accepted = {}
  for (const [id, keys] of Object.entries(value)) {
    ...
    if (!Array.isArray(keys)) return { ok: false, error: `${field}[${id}] must be an array of price keys` }
    const named = []
    for (const key of keys) {
      if (!PRICE_KEYS.includes(key)) return { ok: false, error: `${field}[${id}] names an unknown price key: ${String(key).slice(0, 20)}` }
      if (!named.includes(key)) named.push(key)
    }
    if (named.length > 0) accepted[id] = named
  }
 ...
 *
 * `adopt` and `keep` are the two answers to a disagreement between a typed price
 * and a published one: adopt hands those fields back to the catalogue, keep
 * records that the typed value was chosen with that official number in view.
 *
...
export function validateOverrides(body) {
  ...
  for (const field of ['adopt', 'keep']) {
    if (body[field] === undefined) continue
    const validated = validateReviewKeys(body[field], field)
    if (!validated.ok) return validated
    patch[field] = validated.value
  }
