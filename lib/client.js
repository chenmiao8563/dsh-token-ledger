...
.tl-bill-head, .tl-bill-row, .tl-bill-total { display: grid; grid-template-columns: minmax(120px, 1fr) 90px 96px 88px 88px 76px 62px; align-items: center; gap: 6px; font-size: 12px; }
@supports (grid-template-columns: subgrid) {
  /* The trailing 1fr is deliberately empty: it takes the width the numbers no longer
     ...
     full width. */
  .tl-bill-table { display: grid; grid-template-columns: minmax(120px, 360px) 90px 96px 88px 88px 76px 62px 1fr; column-gap: 6px; align-items: center; }
      ...
      const rowElement = (row) =>
        h(
          ...
          h(
            ...
            h('span', { className: 'tl-bill-label', title: pricedFrom(row) ?? row.label }, row.label ?? t('billPayAsYouGo')),
