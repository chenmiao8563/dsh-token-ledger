...
.tl-period { margin-left: 6px; font-size: 10px; padding: 0 5px; border-radius: 999px; border: 1px solid rgba(127,127,127,.35); opacity: .75; font-style: normal; }
...
`
    ...
    /**
     * Format a price: a vendor's own quote, converted for display when it needs it.
     *
     ...
     *
     * The conversion is display only, and only for a price quoted in something
     * other than the display currency: DeepSeek and Tencent publish in yuan, so
     * their numbers are already what the table shows and are passed through
     * untouched. Nothing here multiplies a token count by a price.
     *
     ...
     */
    function price(value, rate = null, currency = 'USD') {
      if (typeof value !== 'number' || !Number.isFinite(value)) return '—'
      if (currency === 'CNY') return `¥${trimmed(value)}`
      const converted = rate === null ? value : value * rate
      return `${rate === null ? '$' : '¥'}${trimmed(converted)}`
    }

    /**
     * A number at the precision its magnitude deserves, with trailing zeros cut.
     *
    ...
    const VENDOR_LOGOS = {
      ...
      'openai': { viewBox: '0 0 180 180', color: '#0f0f0f', paths: [
      ...
      'qwen': { viewBox: '0 0 24 24', color: '#6950EF', paths: [
      ...
      'x-ai': { viewBox: '0 0 24 24', color: '#000000', paths: [
        'M14.234 10.162 22.977 0h-2.072l-7.591 8.824L7.251 0H.258l9.168 13.343L.258 24H2.33l8.016-9.318L16.749 24h6.993zm-2.837 3.299-.929-1.329L3.076 1.56h3.182l5.965 8.532.929 1.329 7.754 11.09h-3.182z',
      ] },
      // https://z-cdn.chatglm.cn/z-ai/static/logo.svg
      'z-ai': { viewBox: '0 0 30 30', color: '#111111', paths: [
        'M24.51,28.51H5.49c-2.21,0-4-1.79-4-4V5.49c0-2.21,1.79-4,4-4h19.03c2.21,0,4,1.79,4,4v19.03 C28.51,26.72,26.72,28.51,24.51,28.51z',
      ...
      'kimi': { viewBox: '0 0 24 24', color: '#000000', paths: [
      ...
      'minimax': { viewBox: '0 0 24 24', color: '#E73562', paths: [
      ...
      'bytedance': { viewBox: '0 0 24 24', color: '#3C8CFF', paths: [
     ...
     *
     * @param {unknown} vendor - the vendor id.
     * @returns {object} the element: an SVG for a known vendor, letters otherwise.
     */
    function vendorLogo(vendor) {
      ...
      return h(
        ...
        h(
          'svg',
          { viewBox: logo.viewBox, width: 14, height: 14, role: 'img', 'aria-label': vendorName(id) },
          logo.paths.map((d, index) => h('path', { key: index, d, fill: logo.color })),
        ),
      )
    }
          ...
          isEditing
            ...
            : PRICE_FIELDS.map((field) =>
                h(
                  ...
                  price(model.prices?.[field.key], rate, currency),
                ),
              ),
          h(
            'span',
