...
.tl-bill-scroll { overflow: auto; max-height: 460px; }
.tl-bill-table { min-width: 640px; display: flex; flex-direction: column; }
.tl-bill-head, .tl-bill-row, .tl-bill-total { display: grid; grid-template-columns: minmax(160px, 1fr) 76px 96px 92px 88px 112px; align-items: center; gap: 8px; font-size: 12px; }
...
`
    ...
    /**
     * The bill: the same tokens as the overview, priced and totalled.
     *
     ...
     * A price that was matched by name rather than by exact id is listed too, because
     * a join that guessed should be checkable. And the money column is tabular and
     * fixed-width, because a bill is read by comparing numbers down it.
     *
     * @param {object} props - the view inputs.
     * @param {(key: string) => string} props.t - the bound translator.
     * @param {{ status: string, data: object|null, error: string|null }} props.state - what to show.
     * @param {string} props.by - the grouping: vendor, model, workspace, session or subscription.
     * @param {string} props.range - the period: week, month, year or all.
     * @param {(value: string) => void} props.onBy - grouping change handler.
     * @param {(value: string) => void} props.onRange - period change handler.
     * @returns {object} the element.
     */
    function BillView({ t, state, by, range, onBy, onRange }) {
      const header = h(
        'div',
        { className: 'tl-head' },
      ...
      if (state.data === null) {
        return h(
          ...
          h(
            ...
            { className: 'tl-card' },
            h('h3', { className: 'tl-card-title' }, state.status === 'error' ? t('unavailable') : t('loading')),
            state.status === 'error' ? h('p', { className: 'tl-note' }, t('billUnavailableReason')) : null,
            state.error === null ? null : h('p', { className: 'tl-error' }, state.error),
          ),
        )
      }
      ...
      const money = (value) => `${symbol}${Number(value ?? 0).toFixed(2)}`
      // The export reads the same route with the same parameters, so the file is what
      // the page is showing rather than a second rendering of it.
      const exportUrl = (format) => `${BILL_ENDPOINT}?by=${encodeURIComponent(by)}&range=${encodeURIComponent(range)}&format=${format}`
      const stamp = new Date(bill.generatedAt ?? Date.now()).toISOString().slice(0, 10)
      const rowElement = (row) =>
        h(
          ...
          // the two are alternatives, so they must not look like a sum.
          h(
            'span',
            { className: 'tl-bill-cost', title: row.plan === true ? t('billCoveredUsage') : null },
            h('span', null, money(row.cost)),
            row.plan === true && typeof row.usageCost === 'number'
              ? h('em', { className: 'tl-bill-sub' }, `${t('billCoveredUsage')} ${money(row.usageCost)}`)
              : null,
          ),
        )
      ...
      const table = h(
        ...
        h(
          ...
          h(Tabs, {
            ...
            onChange: onBy,
            options: [
              { value: 'vendor', label: t('billByVendor') },
              { value: 'subscription', label: t('billBySubscription') },
              { value: 'workspace', label: t('billByWorkspace') },
              { value: 'session', label: t('billBySession') },
              { value: 'model', label: t('billByModel') },
            ],
          }),
          h('div', { className: 'tl-spacer' }),
          h(Tabs, {
          ...
          // The export button sits at the top right, where a bill's export belongs.
          h(
            'div',
            ...
            h(
              'a',
              {
                className: 'tl-btn',
                href: exportUrl('csv'),
                download: `token-bill-${by}-${stamp}.csv`,
                title: t('billExportCsv'),
              },
              t('billExportCsv'),
            ),
            h(
              'a',
              {
                className: 'tl-btn',
                href: exportUrl('json'),
                download: `token-bill-${by}-${stamp}.json`,
                title: t('billExportJson'),
              },
              t('billExportJson'),
            ),
          ),
        ...
        rows.length === 0
          ? h('p', { className: 'tl-note' }, t('billEmpty'))
          : h(
              'div',
              ...
              h(
                ...
                h(
                  ...
                  h('span', null, t('billBy' + by.charAt(0).toUpperCase() + by.slice(1))),
                  h('span', null, t('calls')),
                ...
                h(
                  ...
                  h('span', null, money(totals.totalCost)),
                ),
              ),
            ),
      )
      ...
      notes.push(
        h(
          ...
          `${t('billEstimate')}: ${t('source')} ${bill.priceSource}${bill.fxRate === null ? '' : ` · 1 USD = ${rateText(bill.fxRate)} ${bill.currency}`}`,
        ),
      )
      if (Array.isArray(bill.subscriptions) && bill.subscriptions.length > 0) {
        for (const plan of bill.subscriptions) {
          notes.push(
            h(
              'p',
              { key: `plan-${plan.plan}`, className: 'tl-note' },
              `${plan.plan}${plan.vendor === null ? '' : ` (${vendorName(plan.vendor)})`} · ${t('billAmount')} ${plan.currency} ${plan.amount} · ${t('billTotal')} ${money(plan.share)}`,
            ),
          )
        }
      }
      if (Array.isArray(bill.unpriced) && bill.unpriced.length > 0) {
        notes.push(
          h(
            'div',
            ...
            h(
              ...
              bill.unpriced.slice(0, 8).map((row) =>
                h(
                  'li',
                  { key: row.model },
                  `${row.model} — ${compact(row.tokens)} (${row.calls} ${t('calls')}) · ${row.reason}`,
                ),
              ),
            ),
          ),
      ...
      const named = (Array.isArray(bill.joins) ? bill.joins : []).filter((join) => join.confidence !== 'exact')
      if (named.length > 0) {
        notes.push(
          h(
            'div',
            ...
            h(
              'ul',
              { className: 'tl-bill-note-list' },
              named.slice(0, 8).map((join) =>
                h('li', { key: join.model }, `${join.model} → ${join.vendor}/${join.priceId} (${join.matchedOn})`),
              ),
            ),
          ),
        )
    ...
    function TokenLedgerSection({ t }) {
      ...
      const [billBy, setBillBy] = react.useState('vendor')
      const [billRange, setBillRange] = react.useState('month')
      const [bill, setBill] = react.useState({ status: 'idle', data: null, error: null })
      ...
      // The bill is refetched when the grouping or the period changes, because both
      // are the host's to compute: pricing tokens is arithmetic the page has no
      // prices for, and a second implementation of it in the browser would be a
      // second answer.
      react.useEffect(() => {
        if (kind !== 'bill') return undefined
        let cancelled = false
        const url = `${BILL_ENDPOINT}?by=${encodeURIComponent(billBy)}&range=${encodeURIComponent(billRange)}`
        const load = () => {
          fetch(url, { cache: 'no-store' })
            ...
            .then((data) => {
              if (!cancelled) setBill({ status: 'ready', data, error: null })
            })
            .catch((error) => {
              if (cancelled) return
              setBill((previous) =>
      ...
      return h(
        ...
        kind === 'rates'
          ...
          : kind === 'bill'
            ? h(BillView, { t, state: bill, by: billBy, range: billRange, onBy: setBillBy, onRange: setBillRange })
