...
.tl-rate-head, .tl-rate-row { display: grid; grid-template-columns: minmax(150px, 1fr) repeat(4, 82px) 96px; align-items: center; gap: 8px; font-size: 12px; }
.tl-rate-head { font-size: 10px; opacity: .5; }
.tl-rate-head > span { text-align: right; }
.tl-rate-head > span:first-child { text-align: left; }
/* The table scrolls sideways rather than squeezing the model name out of
   existence: at a fixed 470px of price and action columns a narrow panel left
   the name column zero pixels wide, so the row showed prices for a model nobody
   could identify. The name column has a floor now and the table scrolls under
   it. */
.tl-rate-scroll { overflow-x: auto; overflow-y: hidden; padding-bottom: 2px; }
.tl-rate-table { min-width: 552px; display: flex; flex-direction: column; }
            ...
            : PRICE_FIELDS.map((field) =>
                h(
                  'span',
                  {
                    ...
                    // cannot trace.
                    title: rate === null ? undefined : `${t('priceUsdHint')} ${price(model.prices?.[field.key], null)}`,
                  },
