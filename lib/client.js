...
.tl-rates-list { display: flex; flex-direction: column; gap: 12px; max-height: 460px; overflow-y: auto; padding-right: 4px; }
...
.tl-rate-head, .tl-rate-row { display: grid; grid-template-columns: minmax(0, 1fr) repeat(4, 80px) 150px; align-items: center; gap: 8px; font-size: 12px; }
...
.tl-rate-actions { display: flex; align-items: center; justify-content: flex-end; gap: 6px; }
...
`
    ...
    /**
     * Format a price: US dollars per one million tokens.
     *
     ...
     *
     * @param {unknown} value - USD per 1M tokens, or null when unpriced.
     ...
     * @param {object} drafts - the four draft strings.
     * @returns {object|null} the patch, or null when a field is not a price.
     */
    function pricePatch(id, drafts) {
      const prices = {}
    ...
    function draftsOf(model) {
      ...
      for (const field of PRICE_FIELDS) {
        ...
        drafts[field.key] = typeof value === 'number' && Number.isFinite(value) ? String(value) : ''
      }
      return drafts
    }
    ...
    function RatesView({ t, state, onPatch }) {
      ...
      const rowFor = (model) => {
        ...
        return h(
          'div',
          { key: String(model.id), className: 'tl-rate-row', 'data-manual': model.source === 'manual' ? 'true' : 'false' },
          ...
          isEditing
            ...
            : PRICE_FIELDS.map((field) => h('span', { key: field.key, className: 'tl-rate-price' }, price(model.prices?.[field.key]))),
          h(
            'span',
              ...
              ? [
                  h(
                    'button',
                    {
                      ...
                      disabled: busy,
                      onClick: () => {
                        const patch = pricePatch(model.id, editing)
                        if (patch === null) {
                  ...
                  h('button', { key: 'cancel', type: 'button', className: 'tl-btn', onClick: () => setEditing(null) }, t('fxCancel')),
                ]
              : [
                  h('button', { key: 'edit', type: 'button', className: 'tl-btn', onClick: () => setEditing({ id: model.id, ...draftsOf(model), invalid: false }) }, t('edit')),
                  model.source === 'manual'
                    ? h('button', { key: 'clear', type: 'button', className: 'tl-btn', disabled: busy, onClick: () => send({ models: { [model.id]: null } }) }, t('clear'))
                    : null,
                ],
          ),
        )
      ...
      const pricesCard = h(
        ...
        h(
          ...
          h(
            ...
            { className: 'tl-card-title' },
            t('priceTitle'),
            h('span', { className: 'tl-status' }, t('perMillion')),
          ),
          h('div', { className: 'tl-spacer' }),
          h('input', {
        ...
        h('p', { className: 'tl-note' }, catalogue.available === true ? t('offlineNote') : t('neverFetchedHint')),
        groups.length === 0
      ...
      const saveDraft = () => {
        ...
        if (id === '') return
        const patch = pricePatch(id, draft)
        if (patch === null) {
      ...
      const manualCard = h(
        'div',
        { className: 'tl-card' },
        h('h3', { className: 'tl-card-title' }, t('manualFormTitle')),
        h(
          'div',
          ...
          h(
            'button',
            { type: 'button', className: 'tl-btn tl-btn-primary', disabled: busy || draft.id.trim() === '', onClick: saveDraft },
            t('save'),
          ),
        ),
