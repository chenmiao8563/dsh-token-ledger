...
import { RATES_PATH, createOverviewRoute, createRatesRoute, OVERVIEW_PATH } from './route.js'
 ...
 * @param {{ ledgerPath?: string, backfill?: boolean, rates?: boolean|object }} [config] - composition config. `rates: false` disables the pricing feature's networking entirely, leaving hand-entered values as the only source, which is the setting a strictly offline host wants.
 * @returns {void}
 */
  ...
  if (typeof ctx.inject === 'function') {
    ctx.inject(['webServer'], (webCtx) => {
      ctx.effect(() => {
        ...
        log.info('[token-ledger] settings page routes ready at %s and %s', OVERVIEW_PATH, RATES_PATH)
        return () => {
          disposeRates?.()
          dispose?.()
        }
