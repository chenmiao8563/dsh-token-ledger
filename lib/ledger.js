 ...
 *
 * The `cwd` sits on the session's own creation record, and a bill groups by it, so
 * it is read from the events rather than required from the caller: the plugin's
 * backfill and the CLI both hand over a raw event list, and the field is there.
 *
...
export class UsageLedger {
  ...
  adoptSession(session) {
    ...
    if (id === '') return
    this.setWorkspace(id, session.header?.cwd ?? session.cwd ?? session.workspace)
    if (!this.cursors.has(id)) {
