 ...
 * field, and its cursors would stop those days from ever acquiring one. Version
 * 4 adds two things a bill cannot be computed without — a per-day,
 * per-session, per-model cross table carrying the time-of-day split, and the
 * working directory on each session — and neither can be recovered by folding
 * only the events the earlier cursors still allow.
 */
...
/**
 * The window some vendors charge more for, in the vendor's own terms.
 *
 * DeepSeek prices each of its lines twice — a peak price and an off-peak price
 * that is half of it — and states the window as 09:00–12:00 and 14:00–18:00
 * Beijing time on weekdays, everything else being off-peak. The ledger records
 * the split generically because it does not know which vendor a route bills
 * through: a token that fell inside *this* window is marked as such, and a
 * billing view uses the marks only for the vendors that actually charge by time.
 */
const PEAK_ZONE_OFFSET_MINUTES = 8 * 60
const PEAK_WINDOWS = [
  [9 * 60, 12 * 60],
  [14 * 60, 18 * 60],
]
...
/**
 * Whether a moment falls inside the time-of-day window above.
 *
 * @param {unknown} timeMs - event time in milliseconds since the epoch.
 * @returns {boolean} true when a time-priced vendor would charge its peak rate.
 */
export function isPeakTime(timeMs) {
  if (typeof timeMs !== 'number' || !Number.isFinite(timeMs) || timeMs <= 0) return false
  // Shift the epoch by the zone offset and read the UTC parts: the window belongs
  // to Beijing wall-clock time, not to the machine's timezone.
  const local = new Date(timeMs + PEAK_ZONE_OFFSET_MINUTES * 60 * 1000)
...
/**
 * The working directory a stored session was created in.
 *
 * The `cwd` sits on the session's own creation record — the `session` header line
 * of a raw log — and a bill groups by it, so it is read from the events rather than
 * required from the caller: the CLI hands over a raw record list and the field is
 * there.
 *
 * **The plugin's live path cannot rely on this.** DSH's `sessionPersistence` hands
 * out the *sequenced* events plus a separate header, and the header line — the one
 * carrying `cwd` — is not in that array. That is why a host that backfilled from
 * `inspect()` recorded sessions with no working directory at all while a CLI
 * `rebuild` of the same logs recorded every one of them. The host therefore passes
 * the header's `cwd` explicitly, and {@link cwdFrom} is what reads it out.
 *
 * @param {object[]} events - the session's events.
 * @returns {string|null} the directory, or null when the log does not carry one.
 */
export function cwdOf(events) {
  if (!Array.isArray(events)) return null
  for (const event of events) {
    if (event === null || typeof event !== 'object') continue
    for (const value of [event.cwd, event.data?.cwd, event.data?.header?.cwd]) {
...
/**
 * Read a working directory out of whichever of DSH's shapes carries it.
 *
 * A stored session header, a live `Session`, its `header` and its `meta` all
 * describe the same thing under slightly different keys, and which one is present
 * depends on how the session was reached rather than on anything this plugin
 * controls. So every shape is tried, in the order most likely to hold the field,
 * and the first non-empty string wins.
 *
 * @param {unknown[]} sources - candidate objects, any of which may carry a `cwd`.
 * @returns {string|null} the directory, or null when none of them has one.
 */
export function cwdFrom(sources) {
  for (const source of Array.isArray(sources) ? sources : []) {
    if (source === null || typeof source !== 'object') continue
    const candidates = [source.cwd, source.meta?.cwd, source.header?.cwd, source.session?.cwd, source.workspace?.cwd, source.identity?.cwd]
...
export class UsageLedger {
  ...
  reset() {
    ...
    this.models = new Map()
    /** @type {Map<string, { sessionId: string, cwd: string|null, counters: ReturnType<typeof emptyCounters>, calls: number, firstAt: number|null, lastAt: number|null }>} */
    this.sessions = new Map()
    /**
     * The cross table a bill is computed from: one row per day, session and model,
     * with the tokens split by the time-of-day window.
     *
     * The flat tables above answer "how much in total", which is all an overview
     * needs. Money needs more: a vendor with two rates is only priced correctly if
     * the tokens are already separated by when they were spent, and a bill grouped
     * by session or by workspace has to add up per model underneath, because each
     * model has its own price. Neither can be recovered by adding totals together
     * after the fact.
     *
     * @type {Map<string, { date: string, sessionId: string, model: string, calls: number, peak: ReturnType<typeof emptyCounters>, offPeak: ReturnType<typeof emptyCounters> }>}
     */
    this.usage = new Map()
  ...
  sessionRecord(sessionId) {
    ...
    if (record === undefined) {
      record = { sessionId, cwd: null, title: null, counters: emptyCounters(), calls: 0, firstAt: null, lastAt: null }
  ...
  /**
   * Record where a session was working.
   *
   * The working directory is what a workspace bill groups by. It is set once per
   * session and never overwritten by a later `null`, because the first observation
   * comes from the session's own creation record and later ones may simply not
   * carry it.
   *
   * @param {string} sessionId - the session id.
   * @param {unknown} cwd - the working directory.
   * @returns {void}
   */
  setWorkspace(sessionId, cwd) {
    if (typeof cwd !== 'string' || cwd.trim() === '') return
    const record = this.sessionRecord(String(sessionId))
    if (record.cwd === null) record.cwd = cwd
  }

  /**
   * Record the name DSH gave a session.
   *
   * Unlike the working directory this one *is* overwritten: a session is first
   * named from a truncated first prompt and later renamed by a model-written
   * title, and the later one is the name the user sees in the sidebar.
   *
   * @param {string} sessionId - the session id.
   * @param {unknown} title - the session title.
   * @returns {void}
   */
  ...
  /**
   * The day/session/model row a commit belongs to, creating it on first use.
   *
   * @param {string} dateKey - the local day.
   * @param {string} sessionId - the owning session.
   * @param {string} modelKey - the provider/model route.
   * @returns {object} the row.
   */
  usageRecord(dateKey, sessionId, modelKey) {
    const key = `${dateKey}\u0000${sessionId}\u0000${modelKey}`
    let record = this.usage.get(key)
    if (record === undefined) {
      record = { date: dateKey, sessionId, model: modelKey, calls: 0, peak: emptyCounters(), offPeak: emptyCounters() }
  ...
  commit(sessionId, { counters, time, provider = null, model = null, replacementKey }) {
    ...
    if (previous !== undefined && previous.key === replacementKey) {
      ...
      if (previous.usageKey !== undefined) {
        const row = this.usage.get(previous.usageKey)
        if (row !== undefined) subInto(row[previous.period], previous.counters)
      }
      this.calls -= 1
      session.calls -= 1
      ...
      if (previous.usageKey !== undefined) {
        const row = this.usage.get(previous.usageKey)
        if (row !== undefined) row.calls -= 1
      }
    }

    const route = this.routeOf(sessionId)
    const modelKey = `${provider ?? route.provider}/${model ?? route.model}`
    // Which side of a time-priced vendor's window this call fell on. The same
    // moment decides it for every bucket of the call, because the call is one
    // request: a peak request pays the peak rate for its input and its output.
    const period = isPeakTime(time) ? 'peak' : 'offPeak'
    const usageKey = `${dateKey}\u0000${sessionId}\u0000${modelKey}`
    const usageRow = this.usageRecord(dateKey, sessionId, modelKey)
    ...
    addInto(usageRow[period], counters)
    this.calls += 1
    session.calls += 1
    ...
    this.last.set(sessionId, { key: replacementKey, counters, dateKey, modelKey, usageKey, period })
  }
  ...
  consume(sessionId, event) {
    ...
    switch (event.type) {
      case 'session/title': {
        // DSH names a session itself — first from a truncated first prompt, then
        // from a model-written title — and the bill labels a session row with it.
        // The last one seen wins, because the generated title replaces the fallback.
        this.setTitle(sessionId, data.title)
        return
      }
   ...
   * @returns {void}
   */
  adoptHistory({ sessionId, events, inheritedEventCount = 0, cwd = null }) {
    if (!Array.isArray(events)) return
    const id = String(sessionId)
    this.setWorkspace(id, cwd ?? cwdOf(events))
  ...
  adoptSession(session) {
    ...
    if (id === '') return
    // A live Session may carry the directory on itself, on its header or in its
    // meta; and if none of those has it, the first events of a session that has not
    // been written yet are still worth a bounded look, because a missing `cwd` costs
    // a whole grouping on the bill.
    this.setWorkspace(id, cwdFrom([session, session?.header, session?.meta, session?.session]) ?? cwdOf(Array.isArray(session?.events) ? session.events.slice(0, 64) : []))
    if (!this.cursors.has(id)) {
  ...
  snapshot() {
    ...
    return {
      ...
      usage: [...this.usage.values()]
        // A call that was replaced by a later report of the same attempt leaves
        // nothing behind on the day it was first seen. That row is not usage, and a
        // bill should not carry a line with no calls and no tokens.
        .filter((row) => row.calls > 0 || row.peak.totalTokens > 0 || row.offPeak.totalTokens > 0)
        ...
        .map((row) => ({
          ...
          model: row.model,
          calls: row.calls,
          // The total is derived rather than stored, and stored in the snapshot,
          // for the same reason `totalTokens` is: a reader that has to add two
          // sets of four numbers is a reader that can add them wrong.
          ...addCounters(row.peak, row.offPeak),
          peak: { ...row.peak },
          offPeak: { ...row.offPeak },
        })),
  ...
  restore(snapshot) {
    ...
    for (const session of snapshot.sessions ?? []) {
      ...
      record.cwd = typeof session.cwd === 'string' && session.cwd !== '' ? session.cwd : null
      record.title = typeof session.title === 'string' && session.title !== '' ? session.title : null
    ...
    for (const row of snapshot.usage ?? []) {
      ...
      if (date === '' || sessionId === '' || model === '') continue
      const record = this.usageRecord(date, sessionId, model)
      // Read the buckets directly rather than through `absorb`: that helper refuses
      // a zero total, and a side of the window with no usage is a real row.
      for (const period of ['peak', 'offPeak']) {
        for (const key of BUCKET_KEYS) record[period][key] = toCount(row[period]?.[key])
        record[period].totalTokens = BUCKET_KEYS.reduce((sum, key) => sum + record[period][key], 0)
        record[period].reasoningTokens = toCount(row[period]?.reasoningTokens)
      }
      record.calls = toCount(row.calls)
    }
   ...
   *
   * The text begins with a byte-order mark for the same reason the bill's export does:
   * Excel reads a `.csv` in the system code page unless the first bytes tell it the
   * file is UTF-8, so a key that is not ASCII — a session named in Chinese, a model id
   * with a CJK vendor in it — arrives as mojibake without it. See `CSV_BOM` in
   * `bill.js` for the longer version of that note.
   *
  ...
  toCsv(kind = 'daily') {
    ...
    return `${CSV_BOM}${[header, ...rows].map((row) => row.join(',')).join('\r\n')}\r\n`
  }
}
