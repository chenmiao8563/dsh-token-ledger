 ...
 * @param {{ modelsUrl?: string, fxUrl?: string, perVendor?: number, vendorLimit?: number, fetchImpl?: typeof fetch, now?: () => number, intervalMs?: number, log?: object }} [input.options] - overrides, mostly for tests.
...
export function createRatesService({ path, options = {} }) {
  ...
  const refresh = async (trigger = {}) => {
    ...
    inFlight = (async () => {
      ...
      if (models.ok) {
        const parsed = parseCatalogue(models.json, { perVendor, now: at, vendorLimit })
        if (parsed.selected > 0) {
          state.catalogue = {
            ...
            availableVendorCount: parsed.availableVendorCount,
          ...
          outcome.catalogue = `ok (${parsed.selected} models, ${parsed.vendors.length} of ${parsed.availableVendorCount} vendors)`
        } else {
