 ...
 */
export async function fetchJson(url, { fetchImpl = globalThis.fetch, timeoutMs = MODELS_TIMEOUT_MS } = {}) {
    ...
    inFlight = (async () => {
      ...
      if (models.ok) {
        ...
        if (parsed.selected > 0) {
          ...
          outcome.catalogue = `ok (${parsed.selected} models, ${parsed.vendors.length} of ${parsed.availableVendorCount} vendors, ${source})`
        } else {
