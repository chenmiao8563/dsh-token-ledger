 ...
 *
 * Columns are the ones a bill is read by, in the order the page shows them —
 * cache-hit input, cache-miss input, output, hit rate, cost — with the currency on
 ...
 */
export function buildBill({ snapshot, catalogue, subscriptions = [], by = BILL_SECTIONS[0], range = 'month', now = new Date() } = {}) {
  ...
  for (const usage of snapshot?.usage ?? []) {
    ...
    // A model is named the way the bill reads it: whose it is, then what it is —
    // `deepseek/DeepSeek-V4.1-Flash` rather than a price-list name with no owner.
    const modelName = entry?.model ?? routeModel ?? model
    const label = `${vendor}/${modelName}`
    ...
    const dimensionLabel =
      dimension === 'workspace' ? (workspace ?? '(unknown)') : dimension === 'session' ? session.label : key
    const row = byDimension.get(key) ?? blankRow(dimensionLabel, dimension === 'model' ? model : dimension === 'session' ? session.sublabel : null)
