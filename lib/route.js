...
import { billToCsv, buildBill } from './bill.js'
 ...
 * was showing rather than a second implementation of the same arithmetic. The
 * grouping and the range are read from the query, and an unknown value falls back
 * to the default instead of failing: a bill is a read, and refusing it because a
 * link carried a stale parameter would be less useful than answering.
 *
 * @param {object} input - the route's dependencies.
 ...
 * @returns {{ kind: 'exact', path: string, handler: (req: object, res: object) => void }} the route.
 */
export function createBillRoute({ snapshot, catalogue, subscriptions = [], options = {} }) {
  const now = options.now ?? (() => new Date())
  const onError = options.onError ?? (() => {})
    ...
    handler: (req, res) => {
      ...
      try {
        const bill = buildBill({
          ...
          catalogue: catalogue(),
          subscriptions,
          by: query.get('by') ?? 'vendor',
          range: query.get('range') ?? 'month',
          now: now(),
        })
        if ((query.get('format') ?? 'json').toLowerCase() === 'csv') {
          ...
          res.writeHead(200, {
            ...
            'content-length': Buffer.byteLength(text),
            'cache-control': 'no-store',
            'content-disposition': `attachment; filename="token-bill-${bill.by}-${stamp}.csv"`,
          })
          res.end(text)
          return
        }
