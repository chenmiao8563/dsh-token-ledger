...
从 0.5 起，设置页有三个页签：**概览**（账本本身）、**费率**（各厂商最新模型的价格表与
实时美元汇率）、**账单**（把同一批用量折算成钱——按厂商 / 工作区 / 会话 / 模型 / 套餐分组，
可导出 CSV 或 JSON）。账单页会写明价格来源、汇率，以及每一个没能套上价格的模型，
所以这个数字是可以核对的；概览页仍然只数 token，到此为止。详见[设置页](#设置页)。
...
同一批用量，按账单的读法分组，并给出每一组的费用。

- **五种分组**：按**厂商**、按**工作区**、按**会话**、按**模型**、按**套餐**。工作区一行
  就是账本记录的 `cwd`——真正在里面干过活的目录，不是谁手填的标签。每行都带调用次数、
  输入、输出、缓存命中率和这一组的花费。
- **四个区间**：本周 / 本月 / 本年 / 全部，与概览页使用同一套「含今天在内往前推」的定义。
- **按套餐计费且不重复计费**：配置里的 `subscriptions` 填月度套餐（
  ...
  区间内完全没有用量、但确实买了套餐的厂商照样计入，因为钱确实花了。
- **右上角导出 CSV 或 JSON**：两个导出读的是同一条路由、带上页面当前的维度与区间，
  因此导出的文件就是页面本身，而不是另写一套算一遍。CSV 每行都带币种，并有一行 `TOTAL`。
...
- **概览页不算钱，账单页给出写明口径的估算。** 概览页只数 token，到此为止。账单页确实
  会把 token 乘上价格——账单本来就是这个意思；它同时写明价格来源、换算用的汇率、每一个
  靠名字匹配上的价格，以及每一个没能定价的模型，因此这个数字是用来核对的，不是用来信的。
  它不是厂商开给你的账单，也不声称是——具体漏了什么见上面的「账单」一节。
